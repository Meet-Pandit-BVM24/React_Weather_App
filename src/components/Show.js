...
export default function Show() {
    ...
    const fetchByCord = async (lat, lon) => {
        try {
            const url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lng}&units=metric&appid=e2d023b018b931d352b12d9a422483d0`
    ...
    const fetchApi = async () => {
        const url = `https://api.openweathermap.org/data/2.5/weather?q=${search}&units=metric&appid=e2d023b018b931d352b12d9a422483d0`
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error('Network response was not OK');
        }
    ...
    return (
        ...
        <>
            <div className="container"><div className="left-box text-white">
                ...
                <div className="right-box">
                    ...
                    <div className="display2">
                        <div className="text-center">{info.name} , {info.country}</div>

                        <hr /><div className="item ">Temperature  <span>{info.temperature}°C</span> </div>
                        <hr /> <div className="item ">Humidity <span>{info.humidity}%</span> </div>
                        <hr /> <div className="item ">Visibility <span>{info.visibility}mi</span> </div>
                        <hr /> <div className="item "> Wind Speed <span>{info.windspeed}km/h</span> </div>
